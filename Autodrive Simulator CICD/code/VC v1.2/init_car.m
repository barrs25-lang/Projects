...
car.WB = 2.675;     % wheelbase (m)
car.TR = 1.511;     % track (m)

car.Lcar = 4.306;   % length (m)
car.Wcar = 1.770;   % width (m)
car.Hcar = 1.616;   % height (m)


%  --assume CG is halfway between front and rear axles
car.LR = car.WB / 2;    % distance from cg to rear wheel (m)
car.LF = car.WB - car.LR;   % distance from cg to front wheel (m)
...
car.MCAR = Mcar + 4*Mmale + 1*Mfem;   % total mass of car
...
correction = 0.9709;
car.Rtire = (Dtire / 2) * correction; 


%  --define the remaining parameters
car.STEER_RATIO = 0.058221; % delta (deg) = STEER_RATIO * steer_cmd (deg)

car.MAX_TORQUE = 22534;     % maximum torque command (Nm)
car.MIN_TORQUE = 0;         % minimum torque command (Nm)

car.MAX_BRAKE = 0;          % maximum brake command (Nm)
car.MIN_BRAKE = -65534;     % minimum brake command (Nm)

car.MAX_STEER = 540;        % maximum steer command (deg)
car.MIN_STEER = -540;       % minimum steer command (deg)

% tau = 145;                % time constant estimated (sec)
tau = 50;                   % time constant estimated (sec)
car.RRdamp = car.MCAR/tau;  % rolling resistance (linear viscous) damping (Ns/m)

car.Vdead = 0.05;           % deadzone velocity (m/s)
car.Tdead = 5;              % deadzone torque (Nm)


%  --torque dynamics (optimized 1/28/2024)
wn = 18.205;      % Natural Frequency (rad/s) for torque
zeta = 0.79561;   % Damping Ratio for torque

car.TORQUE_NUMS = wn^2;
car.TORQUE_DENS = [1 2*zeta*wn wn^2];
...
car.BRAKE_NUMS = wn^2;
car.BRAKE_DENS = [1 2*zeta*wn wn^2];


%  --steer dynamics (optimized 1/28/2024)
wn = 12.007;      % Natural Frequency (rad/s) for steer
zeta = 0.70241;   % Damping Ratio for steer

car.STEER_NUMS = wn^2;
car.STEER_DENS = [1 2*zeta*wn wn^2];
...
% psi0 = 2*pi*rand(1);    % initial heading (radians)
