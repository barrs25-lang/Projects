...
fr = logspace(-1,log10(FS/2)-1,1000);
...
if 1
    %  --pull data from par init file
    WZ = 2*pi*par_bus.fz_lead;
    ZZ = par_bus.zz_lead;
    WP = 2*pi*par_bus.fp_lead;
else
...
FRFS = freqs(car.STEER_NUMS,car.STEER_DENS,2*pi*fr);

FRFL = freqz(STEER_LEAD_NUMD,STEER_LEAD_DEND,fr,FS);
FRFC = FRFS .* FRFL;

magS_dB = 20*log10(abs(FRFS));
magC_dB = 20*log10(abs(FRFC));
k3S = find(magS_dB<=-3,1,'first');
k3C = find(magC_dB<=-3,1,'first');
frS_3dB = fr(k3S)
frC_3dB = fr(k3C)
fr_ratio = frC_3dB/frS_3dB
...
xline(fr(k3S),'r--','linewidth',2);
xline(fr(k3C),'b--','linewidth',2);
grid on
ylabel('Mag (dB)')
xlabel('Frequency (Hz)')
xlim([fr(2),fr(end)])
ylim([-20,20])
legend('Steer Plant','Lead Filter','Compensated = Steer * Lead', ...
    'location','southwest')
