...
test_nearest_point_to_wpts.m
    This function validates the nearest_point_to_wpts.m function by allowing
    the user to interactively select five waypoints and then continuously
    computing the nearest point on the path from wherever the mouse cursor is.

mouse_pointer_callback.m
    This function is only called by test_nearest_point_to_wpts.m, and handles
    mouse motion callbacks.  Whenever the mouse moves to a new position on the
    screen, this callback is called to evaluate the nearest_point_to_wpts.m
    function and plot the result interactively.
