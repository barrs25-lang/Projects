function [NWP, EWP, HWP, VWP, KWP, ZWP, SWP, GWP, coursename] = get_waypoint_data( ...
    testcase, vstart, vmax, vfinal, max_alat, max_along, randrotate, folder)
%------------------------------------------------------------------------
...
%  folder = name of folder containing waypoint data
%
...
if (nargin < 8), folder = './waypoint_data'; end
...
if (exist(folder) ~= 7)
    ...
    folder = uigetdir('./','Select the folder containing waypoint data');
...
file_to_load = strcat(folder,'/',files{testcase});
